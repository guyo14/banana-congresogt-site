...
def main():
    print("Connecting to DB and fetching tables...")
    conn = get_connection()
    c_df = pd.read_sql("SELECT * FROM congressmen", conn)
    a_df = pd.read_sql("SELECT * FROM attendance", conn)
    s_df = pd.read_sql("SELECT * FROM sessions", conn)
    vot_df = pd.read_sql("SELECT * FROM voting", conn)
    v_df = pd.read_sql("SELECT * FROM votes", conn)
    conn.close()
    ...
    # Replace null block_id with 'independent'
    c_df['block_id'] = c_df['block_id'].fillna(-1)
    ...
    print("Calculating Voting Aggregations...")
    # Voting Stats
    if v_df.empty:
        v_stats = pd.DataFrame(columns=['voting_id', 'total_favor', 'total_contra', 'total_absent', 'votes_difference'])
    else:
        v_stats = v_df.groupby('voting_id').apply(
            lambda x: pd.Series({
                'total_in_favor': (x['vote_type'] == VoteType.in_favor.value).sum(),
                'total_against': (x['vote_type'] == VoteType.against.value).sum(),
                'total_absent': (x['vote_type'] == VoteType.absent.value).sum()
            })
        ).reset_index()
        v_stats['majority'] = np.where(
            v_stats['total_in_favor'] > v_stats['total_against'],
            VoteMajority.in_favor.value,
            np.where(v_stats['total_against'] > v_stats['total_in_favor'], VoteMajority.against.value, VoteMajority.tie.value)
        )
    v_stats.to_csv('../data/voting_aggregated_stats.csv', index=False)
    ...
    v_df['with_majority'] = pd.Series(
        np.where(
            ...
            (v_df['majority'] == VoteMajority.tie.value),
            pd.NA,
            (
                    (
    ...
    print("Calculating Congressman Stats...")
    ...
    if not a_df.empty:
        a_grp = a_df.groupby(['congressman_id', 'period']).apply(lambda x: pd.Series({
            ...
            'attendance_excused': (x['status'] == AttendanceStatus.license.value).sum()
        })).reset_index()
        ...
        a_all = pd.DataFrame(columns=['congressman_id', 'period', 'attendance_present', 'attendance_absent', 'attendance_excused'])

    # Votes aggregates
    if not v_df.empty:
        v_grp = v_df.groupby(['congressman_id', 'period']).apply(lambda x: pd.Series({
            'votes_favor': (x['vote_type'] == VoteType.in_favor.value).sum(),
            'votes_contra': (x['vote_type'] == VoteType.against.value).sum(),
        ...
        v_all = pd.DataFrame(columns=['congressman_id', 'period', 'votes_favor', 'votes_contra', 'votes_absent', 'votes_with_majority'])
    ...
    print("Calculating Group Stats (Rice Index)...")
    def calc_group_stats(group_col, out_file):
        ...
        if not ga_df.empty:
            ...
            ga_all = pd.DataFrame(columns=[group_col, 'period', 'attendance_present', 'attendance_absent', 'attendance_excused'])

        # Votes & Rice index
        if not gv_df.empty:
            v_group_stats = gv_df.groupby([group_col, 'period', 'voting_id']).apply(lambda x: pd.Series({
                'favor': (x['vote_type'] == VoteType.in_favor.value).sum(),
                'contra': (x['vote_type'] == VoteType.against.value).sum(),
                'absent': (x['vote_type'] == VoteType.absent.value).sum()
            })).reset_index()
            v_group_stats['rice'] = abs(v_group_stats['favor'] - v_group_stats['contra']) / (v_group_stats['favor'] + v_group_stats['contra']).replace(0, np.nan)
            
            gv_period = v_group_stats.groupby([group_col, 'period']).apply(lambda x: pd.Series({
                'votes_favor': x['favor'].sum(),
                'votes_contra': x['contra'].sum(),
            ...
            gv_all = pd.DataFrame(columns=[group_col, 'period', 'votes_favor', 'votes_contra', 'votes_absent', 'rice_index'])
        ...
        sum_cols = ['attendance_present','attendance_absent','attendance_excused','votes_favor','votes_contra','votes_absent']
        for c in sum_cols: 
            if c in res.columns:
    ...
    print("Calculating Congressman Similarity...")
    if not v_df.empty:

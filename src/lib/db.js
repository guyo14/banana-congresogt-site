...
CREATE TABLE IF NOT EXISTS votes (
    ...
    PRIMARY KEY (votation_id, congressman_id)
);
...
function ingestData() {
    ...
    const votations = loadCSV('votations.csv');
    const votes = loadCSV('votes.csv');
    const attendance = loadCSV('attendance.csv');

    // Prepare insert statements
    const insertParty = db.prepare('INSERT OR IGNORE INTO parties (id, name) VALUES (@id, @name)');
    const insertDistrict = db.prepare('INSERT OR IGNORE INTO districts (id, name) VALUES (@id, @name)');
    const insertCongressman = db.prepare('INSERT OR IGNORE INTO congressmen (id, first_name, last_name, key, party_id, district_id, photo_url, birth_date, status) VALUES (@id, @first_name, @last_name, @key, @party_id, @district_id, @photo_url, @birth_date, @status)');
    ...
    const insertVotation = db.prepare('INSERT OR IGNORE INTO votations (id, session_id, subject, start_date) VALUES (@id, @session_id, @subject, @start_date)');
    const insertVote = db.prepare('INSERT OR IGNORE INTO votes (votation_id, congressman_id, vote_type, attendance_status) VALUES (@votation_id, @congressman_id, @vote_type, @attendance_status)');
    ...
    insertMany(insertVotation, votations);
